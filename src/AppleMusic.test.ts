...
describe("AppleMusic", () => {
	...
	test("uses logger from config and keeps it in sync", () => {
		const config = new AppleMusicConfig({
			loggerOptions: { level: LogLevel.Debug },
		});
		const appleMusic = new AppleMusic(config);

		expect(appleMusic.config).toBe(config);
		expect(appleMusic.log).toBe(config.logger);

		const customLogger = new Logger({ level: LogLevel.Error });
		appleMusic.log = customLogger;

		expect(config.logger).toBe(customLogger);
		expect(appleMusic.log).toBe(customLogger);
	});
	...
	test("init authenticates client and initializes endpoints", async () => {
		...
		const firstCallArgs = mockGetAuthenticatedAxios.mock.calls[0];
		expect(firstCallArgs[0]).toBe(appleMusic.config);
		...
		// biome-ignore lint/complexity/useLiteralKeys: We're testing a private property here
	...
	test("endpoint methods delegate to underlying endpoint implementations", async () => {
		...
		const suggestionsResult =
			await appleMusic.Suggestions.suggestions(suggestionsParams);
