...
const avaibleDestinations: Record<DestinationName, LogDestination> = {} as Record<DestinationName, LogDestination>;
...
if (isNode) {
    ...
    avaibleDestinations[DestinationName.File] = {
        log: (message?: any, ...optionalParams: any[]) => {
            fs.appendFileSync('app.log', `[LOG] ${new Date().toISOString()} - ${message} ${optionalParams.join(' ')}\n`);
        },
        error: (message?: any, ...optionalParams: any[]) => {
            fs.appendFileSync('app.log', `[ERROR] ${new Date().toISOString()} - ${message} ${optionalParams.join(' ')}\n`);
        },
        debug: (message?: any, ...optionalParams: any[]) => {
            fs.appendFileSync('app.log', `[DEBUG] ${new Date().toISOString()} - ${message} ${optionalParams.join(' ')}\n`);
        },
...
export class Logger {
    ...
    constructor(options?: { destinations?: DestinationName[], level?: LogLevel, customDestinations?: LogDestination[] }) {
        this.selectedDestinations = options?.destinations || [DestinationName.Console];
            ...
            this.selectedDestinations.forEach(destName => {
                ...
                if (dest) {
                    if (isNode || dest.supportsBrowser) {
                        ...
                        console.warn(`Destination ${destName} is not supported in this environment and will be ignored.`);
                    }
                } else {
                    console.warn(`Destination ${destName} is not recognized and will be ignored.`);
                }
            });
        }
    }
    ...
    public enableDestination(name: DestinationName) {
        ...
        if (dest && !this.enabledDestinations.includes(dest)) {
            if (isNode || dest.supportsBrowser) {
                ...
                console.warn(`Destination ${name} is not supported in this environment and cannot be enabled.`);
            }
        }
    }

    public disableDestination(name: DestinationName) {
        this.enabledDestinations = this.enabledDestinations.filter(dest => dest.name !== name);
    }
