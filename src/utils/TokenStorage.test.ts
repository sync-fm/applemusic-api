...
describe("TokenStorage", () => {
	...
	beforeEach(() => {
		...
		config = new AppleMusicConfig({
			loggerOptions: { level: LogLevel.Error },
		});
	});
	...
	describe("constructor", () => {
		it("should initialize with default values", () => {
			const tokenStorage = new TokenStorage(config, mockAxios, 3600000, 10000);
			expect(tokenStorage.valid).toBe(false);
			expect(tokenStorage.expiresIn).toBe(0);
		});

		it("should initialize with custom values", () => {
			const tokenStorage = new TokenStorage(config, mockAxios, 7200000, 20000);
			expect(tokenStorage.valid).toBe(false);
			expect(tokenStorage.expiresIn).toBe(0);
		});
	});
	...
	describe("getToken", () => {
		...
		it("should throw error if yoinkToken fails to find script URL", async () => {
			...
			await expect(tokenStorage.getToken()).rejects.toThrow(
				"Failed to fetch token: Failed to find script asset URL",
			);
		});
		...
		it("should throw error if yoinkToken fails to find token in JS", async () => {
			...
			await expect(tokenStorage.getToken()).rejects.toThrow(
				"Failed to fetch token: Failed to find token in JS",
			);
		});
		...
		it("should throw error if validateToken fails", async () => {
			...
			await expect(tokenStorage.getToken()).rejects.toThrow(
				"Fetched token is invalid: undefined",
			);
		});
		...
		it("should handle axios errors in validateToken", async () => {
			...
			mockAxios.request.mockRejectedValue(
				new Error("Validation network error"),
			);
	...
	describe("valid getter", () => {
		it("should return true when token is valid", () => {
			const tokenStorage = new TokenStorage(config, mockAxios);
			(tokenStorage as any).isTokenValid = true;
