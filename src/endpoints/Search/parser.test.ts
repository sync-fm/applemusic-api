...
import { SearchEndpointParamsDefaults, WithOption, FieldsAlbums } from "./types";
...
describe("Search parser", () => {
    ...
    test("parseSearchParams splits arrays and nested objects", () => {
        const query = "term=hello&extend=artistUrl,lyrics&with=lyrics,subtitles&types=albums,songs&omit[resource]=autos&format[resources]=map&art[music-videos:url]=video&art[url]=cover";
        const result = parser.parseSearchParams(query);
        ...
        expect(result.art).toEqual({ "music-videos": { url: "video" }, url: "cover" });
    });

    test("parseSearchParams handles nested relation collections", () => {
        const query = "fields[artists]=name,artwork&include[albums]=artists&relate[songs]=albums&limit=5";
        const result = parser.parseSearchParams(query);
    ...
    test("buildSearchQuery includes defaults and encodes output", () => {
        ...
        expect(encoded).toContain("types=activities%2Calbums%2Capple-curators%2Cartists");
    });
    ...
    test("buildSearchQuery decodes encoded terms and handles nested arrays", () => {
        ...
        expect(parsed.get("fields[albums]")).toBe(`${FieldsAlbums.ArtistName},${FieldsAlbums.TrackCount}`);
    ...
    test("parseToAppleMusicAPI flattens resource references and normalizes keys", () => {
        const response = {
            resources: {
                ...
                albums: {
                    album1: { id: "album1", type: "albums", attributes: { name: "Album 1" } },
                },
            },
            results: {
                song: {
        ...
        expect(parsed.results.songs?.data[0]).toEqual({ id: "song1", type: "songs", attributes: { name: "Song 1" } });
        expect(parsed.results.songs?.data[1]).toEqual({ id: "song2", type: "songs" });
        expect(parsed.results["music-videos"]?.data).toEqual([{ id: "mv1" }]);
        expect(parsed.results.genres?.data).toEqual([{ id: "genre1", type: "genres" }]);
    });
});