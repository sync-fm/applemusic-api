...
import {
	ExtendOption,
	IncludeOption,
	type MusicVideoParams,
	MusicVideoParamsDefaults,
	type MusicVideoRelationshipOptions,
	type MusicVideoRequestOptions,
	MusicVideoViewName,
	type MusicVideoViewOptions,
	WithOption,
} from "./types";
...
const parseMusicVideo = parseQuery<
	Partial<MusicVideoParams> &
		Partial<MusicVideoViewOptions> &
		Partial<MusicVideoRelationshipOptions>
>({
	factory: () =>
		({}) as Partial<MusicVideoParams> &
			Partial<MusicVideoViewOptions> &
			Partial<MusicVideoRelationshipOptions>,
	handlers: {
		id: (value, result) => {
...
export const parseMusicVideoParams = (query: string): MusicVideoParams =>
	parseMusicVideo(query) as MusicVideoParams;
...
export const buildMusicVideoQuery = (
	...
	buildQueryString<MusicVideoQueryParams>(params, {
		defaults,
		includeDefaults,
		encode,
	});
