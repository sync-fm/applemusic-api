...
import { ExtendOption, IncludeOption, SongsRelationshipParamsDefaults } from "./types";
...
describe("songs parser", () => {
    test("parseSongParams extracts and filters values", () => {
        const query = [
            ...
            "views=more-by-artist,other-versions,invalid",
            "limit=7",
        ].join("&");
        ...
        expect(result).toEqual({
            ...
            views: [View.MoreByArtist, View.OtherVersions],
            limit: 7,
        });
    });
    ...
    test("buildSongQuery respects provided defaults", () => {
        const query = buildSongQuery({}, true, true, SongsRelationshipParamsDefaults);
