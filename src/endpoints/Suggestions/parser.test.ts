...
test("SUGGEST parseSearchParams partial", () => {
	const query =
		"term=hello&limit=5&fields[artists]=name,artwork&include[albums]=artists";
	const result = parseSearchParams(query);
	expect(result).toEqual({
		term: "hello",
		limit: 5,
